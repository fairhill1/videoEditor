...
use crate::fmt::{format_tick_label, nice_tick_interval, truncate_to_width};
use crate::input::DragMode;
use crate::layout::{compute_lane_height, topmost_lane_top};
use crate::quad::Quad;
...
impl State {
    ...
    pub(crate) fn draw_timeline_panel(&mut self, w: f32, h: f32, top_h: f32, t: f64) {
        ...
        for (visual_i, &track_idx) in video_tracks.iter().enumerate() {
            let lane_y = center_y - half_gap - lane_h - visual_i as f32 * (lane_h + TRACK_LANE_GAP);
